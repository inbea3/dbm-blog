...
3. 同一域名下由 Flask 提供前端静态页与 `/api`、`/assets`（文件在 `frontend/assets/`，样式与脚本）。若前后端域名不同，需正确配置 CORS 与 Cookie（本项目默认同源 `credentials`）。头像与正文插图存于 `media_asset`（`GET /api/media/<uuid>`），外链图用 `POST /api/uploads/article-image` 的 JSON `url`。
